...
int main()
{
	...
	while (true)
	{
		...
		if (numPlayers == 2 || currentPlayer == 0)
		{
			...
			while (!validInput)
			{
				...
				if (playField[playerSelection - 1] == ' ')
				{
					...
					currentPlayer == 0
						? player1Score *= valueField[playerSelection - 1]
						: player2Score *= valueField[playerSelection - 1];
					validInput = true;
					numberFilledBoxes++;
				}
			}
		...
		if (CalculateWinner(player1Score, currentPlayer) || CalculateWinner(player2Score, currentPlayer) || GameIsDraw(
			numberFilledBoxes))
		{
			drawPlayingField(playField);
			system("pause");
